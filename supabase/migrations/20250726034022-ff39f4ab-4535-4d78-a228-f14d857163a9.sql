...
-- For offers that don't have a brand, we need to assign them to a vendor
-- Let's create a default vendor for orphaned offers
INSERT INTO public.vendors (name, email) 
VALUES ('Default Vendor', 'default@vendor.com')
ON CONFLICT DO NOTHING;

-- Get the default vendor ID and assign it to offers without vendor_id
UPDATE public.offers 
SET vendor_id = (SELECT id FROM public.vendors WHERE name = 'Default Vendor' LIMIT 1)
WHERE vendor_id IS NULL;

-- Now drop the brand_id column
ALTER TABLE public.offers DROP COLUMN brand_id CASCADE;

-- Make vendor_id not null now that all offers have a vendor

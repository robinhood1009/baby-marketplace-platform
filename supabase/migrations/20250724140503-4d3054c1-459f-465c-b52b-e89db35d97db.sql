...
-- Add vendor_id to profiles table first
...
USING (id IN (
  ...
  WHERE user_id = auth.uid() AND role = 'vendor'::user_role
));
...
-- Drop existing offers policies first
...
ALTER TABLE public.offers DROP COLUMN IF EXISTS vendor_id CASCADE;
ALTER TABLE public.offers DROP COLUMN IF EXISTS brand CASCADE;
ALTER TABLE public.offers ADD COLUMN brand_id UUID REFERENCES public.brands(id);

-- Create new offers policy for brands
...
USING (brand_id IN (
  ...
  WHERE p.user_id = auth.uid() AND p.role = 'vendor'::user_role
));

-- Drop existing ads policies first
...
ALTER TABLE public.ads DROP COLUMN IF EXISTS vendor_id CASCADE;
ALTER TABLE public.ads ADD COLUMN vendor_id UUID REFERENCES public.vendors(id);

-- Create new ads policies
